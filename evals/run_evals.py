...
def eval_vendor_trust_boundary() -> Eval:
    ...
    plan = store.build_plan(IntakeRequest(
        equipment="standard_wheelchair", plan_id="HUM-MA-PPO", zip="78704",
        recent_visit=True, has_order=False, confidence=0.95))
    ...
    e.check("top pick is in-stock + in-network",
            plan.vendors.shortlist and plan.vendors.shortlist[0].id in IN_STOCK_IN_NETWORK)
    e.check("out-of-network supplier (sup_003) is EXCLUDED, not shortlisted",
            OUT_OF_NETWORK not in short_ids and OUT_OF_NETWORK in excl_ids)
    e.check("no-assignment supplier (sup_004) is EXCLUDED",
            NO_ASSIGNMENT not in short_ids and NO_ASSIGNMENT in excl_ids)
    e.check("backordered supplier (sup_002) is never ranked #1",
...
def eval_no_in_network_escalates() -> Eval:
    ...
    plan = store.build_plan(IntakeRequest(
        equipment="standard_wheelchair", plan_id="CIGNA-MA-HMO", confidence=0.9))
    e.check("shortlist is empty", len(plan.vendors.shortlist) == 0)
    e.check("escalated to human", plan.escalated_to_human)
    e.check("escalation reason mentions no supplier",
...
def eval_low_confidence_escalates() -> Eval:
    ...
    plan = store.build_plan(IntakeRequest(
        equipment="standard_wheelchair", plan_id="HUM-MA-PPO", confidence=0.35))
    e.check("escalated to human", plan.escalated_to_human)
...
def eval_coverage_resolves_from_intake() -> Eval:
    ...
    e.check("clinical need stays unknown (we don't decide it)",
            reqs.get("home_mobility_need") is None)
    e.check("headline never claims coverage",
...
def eval_callback_never_claims_coverage() -> Eval:
    ...
    e.check("does not assert coverage", "you're covered" not in script and "you are covered" not in script)
...
def main() -> int:
    mode = "Claude (claude-opus-4-8)" if os.environ.get("ANTHROPIC_API_KEY") else "deterministic fallback"
